...
def validate_solution(solution, time_window):
    for key, value in solution.items():
        if abs(key.optimal_time - value) > time_window / (60 * 2):
            print('Movement', key.id_number, 'is scheduled outside its time window')
            return False
        for key2, value2 in solution.items():
            if key != key2:
                if key.headway.get(key2.id_number)[0] == 0:
...
print('Initial solution is valid:', validate_solution(initial_solution, 120))
...
for idx in range(EPOCHS):
    for jdx in range(NEIGHBORHOOD_SIZE):
        ...
        new_solution = generate_neighbor_solution(new_solution, affected_movements=3, deviation_scale=50)
...
print('Final solution is valid:', validate_solution(initial_solution, 120))
