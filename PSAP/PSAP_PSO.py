...
from PSAP.Problem import validate_solution, Solver, SolutionGenerator, prepare_movements
...
class ParticleSwarmOptimization(Solver):
    def __init__(self, max_time, vessel_time_window, time_interval, epochs, deviation_scale=45, swarm_size=10,
                 informant_percentage=0.5,
                 alpha=0.95, beta=0.8, gamma=0.8, delta=0.8, epsilon=0.8):
        super().__init__(max_time, vessel_time_window, time_interval)
        self.epochs = epochs
        self.deviation_scale = deviation_scale
        self.swarm_size = swarm_size
        self.informant_percentage = informant_percentage
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.delta = delta
    ...
    def generate_parameters(self, epoch_rng, swarm_size_rng, informant_percentage_rng, alpha_rng, beta_rng, gamma_rng,
                            delta_rng,
                            epsilon_rng):
        self.epochs = rd.randint(epoch_rng[0], epoch_rng[1])
        self.swarm_size = rd.randint(swarm_size_rng[0], swarm_size_rng[1])
        self.informant_percentage = rd.uniform(informant_percentage_rng[0], informant_percentage_rng[1])
        self.alpha = rd.uniform(alpha_rng[0], alpha_rng[1])
        self.beta = rd.uniform(beta_rng[0], beta_rng[1])
        self.gamma = rd.uniform(gamma_rng[0], gamma_rng[1])
        self.delta = rd.uniform(delta_rng[0], delta_rng[1])
        self.epsilon = rd.uniform(epsilon_rng[0], epsilon_rng[1])
        return (self.epochs, self.swarm_size, self.informant_percentage, self.alpha, self.beta, self.gamma, self.delta,
                self.epsilon)
    ...
    def solve(self, movements, precedence=None):
        ...
        # create the swarm
        swarm = create_swarm(movements, self.swarm_size)

        # set the informants
        for particle in swarm:
            particle.set_informants(swarm, self.informant_percentage)
        ...
        if validate_solution(all_time_best_solution, self.vessel_time_window, print_errors=False):
            return all_time_best_solution, all_time_best_obj_val

        # while the time limit is not reached
        while time.time() - start_time < self.max_time and e < self.epochs and not validate_solution(
                all_time_best_solution,
            ...
            for particle in swarm:
                particle.update_velocity(all_time_best_solution, alpha=self.alpha, beta=self.beta, gamma=self.gamma,
                                         delta=self.delta)
                particle.move_position(epsilon=self.epsilon)
        ...
        if validate_solution(all_time_best_solution, self.vessel_time_window, print_errors=True):
            return all_time_best_solution, all_time_best_obj_val
        else:
...
if __name__ == '__main__':
    ...
    solver = ParticleSwarmOptimization(max_time=5, time_interval=time_interval, vessel_time_window=time_window,
                                       epochs=200,
                                       deviation_scale=45, swarm_size=10, informant_percentage=0.5,
                                       alpha=0.95, beta=0.8, gamma=0.8, delta=0.8, epsilon=0.8)
    ...
    while instance < 101:
        ...
        for _ in range(3):
            epochs, swarm_size, informant_percentage, alpha, beta, gamma, delta, epsilon = solver.generate_parameters(
                epoch_rng=[100, 200],
                swarm_size_rng=[10, 20],
                informant_percentage_rng=[0.5, 0.8],
                alpha_rng=[0.8, 0.95],
                beta_rng=[0.7, 0.9],
                gamma_rng=[0.7, 0.9],
                delta_rng=[0.7, 0.9],
